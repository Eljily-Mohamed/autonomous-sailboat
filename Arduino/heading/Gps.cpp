...
void GPS::upDatePosition() {
  ...
  if (!gps.location.isValid()) {
    ...
    if (millis() - gpsMap > 1000) {
      ...
      computeDirectPath(__prevLat, __prevLng);
...
double GPS::getSmoothHeading() {
  if (__status == 2) {
    ...
    for (int i = 0; i < HEADING_HISTORY_LEN; i++) {
      // Convertir les caps en radians et calculer les composantes x et y
      float radians = __headingHistory[i] * PI / 180.0;
      x += cos(radians);
      y += sin(radians);
    }

    // Calculer le cap moyen en radians
    float averageRadians = atan2(y, x);
